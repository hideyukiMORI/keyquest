...
export function resolveQuestResources(options: {
  ...
  const mpGained = calculateMpGain(options.score);
  const focusCrystalGain = Math.floor(options.xpGained / XP_PER_FOCUS_CRYSTAL);
  const repairShardGain = options.score.mistakes;
